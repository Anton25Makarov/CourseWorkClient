...
public class UserReportsAlreadyEvaluatePerformancesWindow extends JFrame {
    ...
    public UserReportsAlreadyEvaluatePerformancesWindow(JFrame parent, Socket socket,
    ...
    private String createReport() {
        ...
        textReport.append("\tВы оценили следующих спотсменов:\r\n");
        int counter = 1;
        for (Sportsman sportsman : sportsmen) {
            ...
            if (!sportsman.getPerformance().getMarks().isEmpty() &&
                ...
                Mark mark = sportsman.getPerformance().getMarks().get(entryReferee);
                if (entryReferee.getSport().equals("Фигурное катание")) {
                    textReport.append(" - За технику: ").append(((FigureSkatingMark) mark).getTechnicalMark());
                    textReport.append(", за артистизм: ").append(((FigureSkatingMark) mark).getPresentationMark());
                    textReport.append(".\r\n");
                }
                if (entryReferee.getSport().equals("Дайвинг")) {
                    textReport.append(" - Оценка: ").append(((DivingMark) mark).getMark());
                    textReport.append(".\r\n");
                }
                if (entryReferee.getSport().equals("Прыжки с трамплина")) {
                    textReport.append(" - Оценка: ").append(((SkiJumpingMark) mark).getMark());
                    textReport.append(".\r\n");
                }
                counter++;
            }
        ...
        if (counter == 1) {
            textReport.append("Пока вы оценили ни одно выступление.");
        }
