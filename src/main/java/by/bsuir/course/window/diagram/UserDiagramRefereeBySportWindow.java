...
public class UserDiagramRefereeBySportWindow extends JFrame {
    ...
    public UserDiagramRefereeBySportWindow(JFrame parent, Socket socket,
    ...
    private void init() {
        ...
        JFreeChart chart = ChartFactory.createPieChart("Соотношение рефери по спорту",
...
//        panel.add(jfxPanel);
    ...
    private PieDataset createDataset() {
        ...
        for (Referee referee : referees) {
            sports.add(referee.getSport());
            System.out.print(referee.getSport() + ", ");
        }
        ...
        for (String string : countriesList) {
            int total = 0;
            for (Referee referee : referees) {
                if (string.equals(referee.getSport())) {
                    total++;
                }
            }
            counts.add(total);
